$(document).ready(function(){
        ...
        $(".bar-container").fadeIn('fast', function() {
            $(".bar-total").stop().css({"width": 0}).animate({
              ...
              complete: function() {
                ...
                $(".bar-check").css({
                    "background-color": "rgba(203, 56, 56, 0.8)"
                });
                $(".bar-total").stop();
                // setTimeout(function(){
                    $(".bar-container").fadeOut('fast', function() {
                        $(".bar-total").css("width", 0);
                        $(".bar-check").css({"background-color": "rgba(255, 217, 48, 0.8"});
                        $.post('https://qbr-skillbar/Check', JSON.stringify({
                            success: false
                        }));
                    });
                // }, 1000);
    ...
    Skillbar.Check = function(data) {
        ...
        if (Percentage + 2 >= Minimum && Percentage - 2 <= Check) {
            $(".bar-check").css({
                "background-color": "rgba(255, 217, 48, 0.8)"
            });
            $.post('https://qbr-skillbar/Check', JSON.stringify({
                success: true
            }));
